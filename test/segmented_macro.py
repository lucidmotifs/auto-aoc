...
if os.path.exists(filename):
    ...
    for i, segment in enumerate(segments):
        print('Press F2 to play segment {}/{}'.format(i+1, len(segments)))
        print('Duration: {:.02} seconds'.format(segment[-1].time - segment[0].time))
        keyboard.wait('F2')
        keyboard.play(segment)

else:
    print('Press F2 to save this fragment. Press F3 to discard it. Press F4 to stop recording.')
    ...
    def handle_event(event):
        ...
        if keyboard.matches(event, 'F2'):
            if event.event_type == keyboard.KEY_DOWN:
                if segment:
                    segments.append(segment)
                segment = []
                print('Saved', len(segments))
        elif keyboard.matches(event, 'F3'):
            if event.event_type == keyboard.KEY_DOWN:
                segment = []
                print('Discarded')
        else:
